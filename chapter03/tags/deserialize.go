...
// DeSerializaStructStrings 함수는 사용자 정의 직렬화 포맷을 사용하여 직렬화된 문자열을 구조체로 변환한다.
func DeSerializeStructStrings(s string, res interface{}) error {
	...
	//직렬화된 문자열을 분할(분리)하여 맵(map)에 저장한다.
	...
	for i := 0; i < r.NumField(); i++ {
		...
		//serialize가 설정되었는지 확인한다.
		if serialize, ok := field.Tag.Lookup("serialize"); ok {
			//"-"를 무시한다. 그렇지 않으면 전체 값이 직렬화 '키(key)'가 된다.
			if serialize == "-" {
				continue
			}
			//맵에 있는지 확인한다.
