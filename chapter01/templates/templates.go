...
const sampleTemplate = `
    ...
    {{ range $index, $item := split .Words ","}}
        {{$index}}: {{$item}}
    {{end}}
    Block are a way to embed templates into one another
    {{ block "block_example" .}}
        No Block defined!
    {{end}}
    {{/*
...
func RunTemplate() error {
    ...
    //블록의 예시를 보여주기 위해 첫 번쨰 템플릿을 복제한 후, 두 번쨰 템플릿을 파싱(parsing)하여 또 다른 템플릿을 생성한다.
    t2, err := t.Clone()
    if err != nil {
        return err
    }
